...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["dangky"])) {
    ...
    if ($hoten == "" || $email == "" || $ngaysinh == "" || $gioitinh == "" || $loaikhachhang == "") {
        
    } else {
        // Sử dụng Prepared Statement để chống SQL injection
        $stmt = $conn->prepare("INSERT INTO customer (hoten, email, sdt, ngaysinh, diachi, gioitinh, loaikhachhang) 
                                VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssssss", $hoten, $email, $sdt, $ngaysinh, $diachi, $gioitinh, $loaikhachhang);
...
<body>
    <form method="post" action="">
        ...
        <p>Loại khách hàng (*)</p>
        <select name="loaikhachhang" required>
